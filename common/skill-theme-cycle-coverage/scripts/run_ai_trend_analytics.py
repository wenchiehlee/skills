...
def owner_for(flag: str) -> str:
    ...
    if flag == "missing_performance":
        return "skill-theme-cycle-index"
    return "skill-theme-cycle-coverage"
...
def write_coverage_md(root: Path, df: pd.DataFrame) -> None:
    lines = [
        ...
        f"Generated: {now_cst()}",
        "",
        "This file is generated by `skill-theme-cycle-coverage` from canonical cycle mapping, performance, segment weights, QA, and freshness outputs.",
        "",
        "## Summary",
        "",
        f"- Rows: `{len(df)}`",
...
def write_inference_md(root: Path, coverage: pd.DataFrame, issues: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"Generated: {now_cst()}",
        "",
        "This report is generated by `skill-theme-cycle-coverage`. It is intentionally constrained by the coverage matrix and issue register; low-quality evidence is not promoted into high-confidence conclusions.",
        "",
        "## Quality Gate",
        "",
