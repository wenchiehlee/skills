...
def built_segment_weight_summary(root: Path) -> tuple[int, int]:
    mapping_path = root / "output" / "company_cycle_mapping.csv"
    major_path = root / "output" / "company_cycle_major_weights.csv"
...
def build_insights(root: Path, raw_latest: str, raw_count: int) -> str:
    index_path = root / "output" / "company_cycle_intensity_taiwan.csv"
    by_symbol_path = root / "output" / "company_cycle_intensity_by_symbol_taiwan.csv"
    ...
    segment_path = root / "data" / "company_segment_weights.csv"
    segment_snapshot_note = "`data/company_segment_weights.csv` 尚無可讀取的 Taiwan segment snapshot；所有混合營收拆分都應視為缺口。"
    if segment_path.is_file():
        seg = pd.read_csv(segment_path)
        if "market" in seg.columns:
            seg = seg[seg["market"].fillna("Taiwan").eq("Taiwan")].copy()
        if not seg.empty and {"stock_code", "source_period"}.issubset(seg.columns):
            period_counts = seg.dropna(subset=["stock_code", "source_period"]).assign(
                stock_code=lambda d: d["stock_code"].astype(str),
                source_period=lambda d: d["source_period"].astype(str),
            ).groupby("stock_code")["source_period"].nunique()
            single_snapshot = int((period_counts == 1).sum())
            multi_snapshot = int((period_counts > 1).sum())
            total_weighted = int(period_counts.shape[0])
            segment_snapshot_note = (
                f"segment weights 口徑：目前 Taiwan active snapshot 覆蓋 `{total_weighted}` 家公司，其中 `{single_snapshot}` 家只有單一 `source_period`、`{multi_snapshot}` 家有多期權重。"
                "若 single snapshot 被套用到長期月營收序列，該段 cycle intensity 只能視為 exposure proxy；"
                "需要用季度 MD/IR evidence 補齊後，YoY 才能被視為較可比的 product-mix adjusted signal。"
            )
    ...
    def anomaly_summary() -> str:
        ...
        quality_notes = []
        if "yoy_data_quality" in df.columns:
            latest_quality = df[df["month"].astype(str).eq(latest_month)]
            mismatch = latest_quality[latest_quality["yoy_data_quality"].astype(str).eq("segment_source_period_mismatch")]
            if not mismatch.empty:
                quality_notes.append("YoY 權重口徑不可比：" + "、".join(f"`{cycle}`" for cycle in mismatch["canonical_cycle"].astype(str).head(6)))
        if quality_notes:
...
def main() -> int:
    ...
    print(
        ...
        f"company_cycle_major_weights.csv rows: {major_weight_rows}"
    )

    derived_files = [
        root / "output" / "company_cycle_intensity_taiwan.csv",
        root / "output" / "company_cycle_intensity_by_symbol_taiwan.csv",
    ]
