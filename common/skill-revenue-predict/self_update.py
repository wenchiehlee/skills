...
使用方式：在每個 skill 的入口程式（如 generate_revenue_predict.py）最前面加入
...
# 本檔案所在的父目錄即為 skill 子目錄名稱（skill-revenue-predict）
...
def _perform_update():
    ...
    if scripts_dir.is_dir():
        # 下載所有 .py 檔案（列舉本地已有的檔名，遠端新增的檔案需手動加入下載清單）
...
def _needs_update() -> bool:
    # 簡化邏輯：只要遠端 SKILL.md 可取得即認為需要更新（不比較版本）
    try:
