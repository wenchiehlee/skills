...
description: 法說會/earnings call 重點萃取與投資影響分析。當使用者要求分析、摘要或 digest 台股/美股公司法說會，或需要從 FIN.srt、音檔、IR、Q&A、第三方逐字稿與前後期資料生成/修正 GT.srt 時使用；找出預期差、模型修正路徑、Q&A 壓力訊號、管理層可信度、股價影響因素，並在美股案例列出台股供應鏈/市場 read-through。
---

# 法說會重點萃取與投資影響分析 SOP (Investor Conference Digest SOP)

本技能用於分析 `InvestorConference` 資料庫中的上市櫃公司或美股公司法說會/earnings call 資料。輸出目標不是「整理管理層說了什麼」，而是判斷：
...
5. 以 `_alphaspread_transcript.md`、`_yahoo_transcript.md`、`_alphamemo_transcript.md` 等第三方逐字稿補足 speaker、英文專有名詞、Q&A 段落與語句順序。
...
8. 保留 FIN 的時間軸，除非有音檔依據可重新對齊；大幅改動時間戳必須說明。
9. 產出 `{StockID}_{Year}_q{N}_GT.srt` 後重跑 `lint_sources.py`。
...
* 可在有本季 AlphaSpread/Yahoo/官方逐字稿、IR 與 Q&A 支持時，建立 `conservative_from_FIN` GT candidate。
* FIN 時間戳只能標示為 approximate scaffold；引用時應優先引用文字來源位置，時間戳僅供閱讀導覽。
* GT metadata 必須包含 `Audio-Checked: none`、`Correction-Sources`、`Timing-Note` 與 `Confidence: medium/low`。
