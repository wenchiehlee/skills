...
- **competitor 是 company 底下的一個 action，不是獨立 domain**：以單一公司為錨點找出其競爭者/同業，且輸出的分類詞彙是這支 skill 自己發明、不依賴任何主題頁既有分組（例如判斷「A 公司的競爭者有哪些」時只看營收/產品重疊，不查任何 theme JSON），屬於 `company` domain；「跨主題頁維護一群公司的競爭關係分組」（例如 `skill-theme-competitor-groups-curate`）主體是主題頁而非單一公司，命名應改用 `theme` domain，而非 `my-tw` 這類未定義的 domain。
- **判準不是「輸出主鍵是不是單一公司」，是「輸出詞彙是否依附某個 theme 的分類體系」**：`skill-theme-competitor-analysis` 雖然以單一股票為輸入、逐股輸出一份 CSV（`output/focus/{stock}/company_competitor_analysis_{stock}.csv`，主鍵看起來是公司），但它輸出的 `relationship_type` 詞彙（`brand_competitor`/`foundry_competitor`/`odm_peer`/`server_peer`/`chip_competitor`）本身就是 `skill-theme-competitor-groups-curate` 各主題頁 `competitive_groups` 在用的同一套分類體系——離開主題頁的分組定義，這些標籤本身沒有意義。這種「把單一公司歸類進某個主題既有詞彙表」的 skill，即使輸入輸出都以單一公司為單位，仍應歸 `theme` domain，因為它的存在意義是替主題頁的分組**提供逐股細節**，不是獨立發明一套公司層級的分類語言（這點不是特例，是跟前面 segment-weight 三層 pipeline 同一條規則的延伸：判斷 domain 看輸出詞彙/交付物依附哪一層概念，不是機械地看一列資料對應幾家公司）。相對地，若一支 skill 分析單一公司的競爭者但用自己發明的標籤（不對照任何 theme 詞彙表），才真的是 `company` domain（例如 `skill-company-competitor-analysis` 這種純粹以財報重疊度自建分類、不查詢任何 `data/themes/*.json` 的工具）。
