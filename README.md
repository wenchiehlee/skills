...
| [skill-company-investorconference-digest](common/skill-company-investorconference-digest) | common | basic | 2.4.6 | 法說會/earnings call 與財報結果 digest SOP；支援 FIN/GT/IR/Q&A、Skills 財報結果、公司財報文件、Yahoo consensus、預期差、模型修正路徑與美股對台股 read-through。 | 2026-09-25 |
| [skill-company-investorconference-ingest](common/skill-company-investorconference-ingest) | common | basic | 1.3.2 | 投資人說明會/財報事件材料蒐集 Ingest 模組；法說會抓音檔、IR、逐字稿，財報事件抓 Skills 財報結果、earnings release、financial tables、SEC filing，並避免對純財報事件產生 FIN/GT。 | 2026-09-25 |
...
| [skill-finmind-fetch](common/skill-finmind-fetch) | common | financial-data | 1.8.0 | Fetch FinMind Taiwan stock margin/price data and quarterly financial-ratio data, exporting GoodInfo Analyzer-compatible CSVs. | 2026-09-23 |
...
| [skill-googlesearch-factsets-fetch](common/skill-googlesearch-factsets-fetch) | common | financial-data | 1.0.0 | 維運 GoogleSearch.Factset 兩階段管線：Stage 0 觀察名單更新（Get觀察名單.py）、Stage 1 Search Group（API key 輪替 + 4 層內容驗證產生 data/md/*.md）、Stage 2 Process Group（品質評分 + 報表 + Google Sheets 上傳），涵蓋 Actions-update-lists.yaml / Actions-search.yaml / Actions-quarantine-files.yaml / Actions-process.yaml 四個 workflow 的手動觸發與除錯，並負責處理與解決 GitHub Issues（每日自動產生的 Quarantine Alert 資料品質議題）。 | 2026-08-26 |
...
| [skill-investment-decision-coach](common/skill-investment-decision-coach) | common | financial-strategy | 1.0.5 | Provide Traditional Chinese expert-level investment decision coaching based on digested book knowledge and an investment methodology library, helping users compare investor and book-derived frameworks, build daily practical investment systems, evaluate opportunities, manage risk, avoid behavioral mistakes, and maintain coherent naming/category taxonomy for finance-related skills. | 2026-09-03 |
| [skill-llm-api-client](common/skill-llm-api-client) | common | library | 1.0.2 | 統一的 LLM 客戶端函式庫（llm），封裝 Gemini API 金鑰輪轉、skill-llm-api-server 的 codex-cli/gemini-cli 橋接、以及本地 MLX 推論，內建 codex → gemini → mlx 自動備援鏈與智慧路由（Smart Routing）。 | 2026-09-01 |
| [skill-llm-api-server](common/skill-llm-api-server) | common | server | 1.1.1 | 在 Synology NAS Docker 容器中運行的 LLM CLI 橋接伺服器，將 OpenAI codex-cli（ChatGPT Pro）與 Google gemini-cli 封裝為 Flask/Waitress HTTP API（/exec、/gemini/exec、/smart/exec），供 llm 函式庫的 CodexProvider 遠端呼叫。 | 2026-08-30 |
...
| [skill-mlx-api-client-ocr](common/skill-mlx-api-client-ocr) | common | document | 1.6.1 | 呼叫 Mac-mini MLX API Server 的 OCR 客戶端（Tailscale 網內），將 PDF 或圖片轉錄為 Markdown 格式，適用於健康報告、稅務文件、財報等各類文件的數位化分析。 | 2026-09-01 |
...
| [skill-mops-fetch](common/skill-mops-fetch) | common | financial-data | 1.2.0 | Fetch Taiwan MOPS data end to end: download quarterly financial-report PDFs, convert same-stem Markdown sidecars with the skill-mac-mini-ocr hybrid PDF/OCR workflow, track official filing deadlines, and orchestrate the repo's batch/early-filer/health-report fetch pipeline. | 2026-09-09 |
...
| [skill-stock-dynamic-valuation-box](common/skill-stock-dynamic-valuation-box) | common | financial-strategy | 1.5.0 | Render no-look-ahead Taiwan-stock time-price diagrams (2–5 year windows) with dynamic TTM P/E valuation boxes (μ±1σ/±2σ), an optional forward-EPS/forward-PE overlay sourced from Yahoo Finance and/or FactSet consensus feeds, and optional matched buy/sell markers, using unadjusted price + statutory-deadline-gated nominal EPS. The μ/σ/±1σ/±2σ PE-band math itself is shared with skill-stock-ma-rsi-bband-macd-peband's calc_pe_band_series() rather than reimplemented here. | 2026-09-23 |
| [skill-stock-fiscal-quarter-resolve](common/skill-stock-fiscal-quarter-resolve) | common | financial-data | 1.1.0 | Deterministic, dependency-free US fiscal-year/quarter resolution shared by skill-company-investorconference-ingest, skill-stock-investorevent-fetch, and ConceptStocks' update_concept_metadata.py. | 2026-09-25 |
| [skill-stock-investorevent-fetch](common/skill-stock-investorevent-fetch) | common | financial-data | 1.2.1 | Regenerate raw_event_upcoming_earnings.csv from TW/US watchlists (MOPS + yfinance), classifying every event as 財報, 法說會, or 受邀法說, with consistent date/fiscal-quarter mapping shared across InvestorConference and InvestorEvents. | 2026-09-25 |
| [skill-stock-ma-rsi-bband-macd-peband](common/skill-stock-ma-rsi-bband-macd-peband) | common | financial-strategy | 1.9.0 | 台股個股/ETF技術指標快照（MA/STD/布林通道、RSI、MACD）與標準 common market PE band；PEBand 用 dated EPS historical PE series，支援 trailing TTM EPS builder、forward EPS、forward consensus EPS、離散PE band分類（classify_pe_band，0~5共6段）。 | 2026-09-23 |
...
| [skill-stock-pipeline-health-monitor](common/skill-stock-pipeline-health-monitor) | common | financial-data | 1.1.0 | 複合技能：委派給所有 skill-*-fetch 抓取技能，維護 Data Health Dashboard 新鮮度、資料系統架構 Repo 角色對照表，並稽核跨 repo CSV 同步 YAML 與 GoodInfo/FinMind 智慧選源。 | 2026-09-25 |
...
| [skill-theme-competitor-analysis](common/skill-theme-competitor-analysis) | common | financial-strategy | 0.1.2 | 依指定 stock id，使用 supply-chain product peer seed 與 relationship rules 區分 brand competitor、ODM peer、server peer 與 supplier/component，並輸出最近三年季度 Revenue、Revenue YoY、Profit、Profit YoY、GM 競爭者分析表。 | 2026-09-24 |
| [skill-theme-competitor-groups-curate](common/skill-theme-competitor-groups-curate) | common | financial-strategy | 0.1.0 | 維護 data/themes/*.json 的 competitive_groups 與 extra_entities，讓 output/themes/*.md 主題頁面依真實產品/商業模式競爭關係分組，並與 data/enrichment_all/*.json 的 relationships.competitors 交叉比對一致性。 | 2026-09-01 |
...
| [skill-tw-legal-rag](common/skill-tw-legal-rag) | common | basic | 1.0.0 | Retrieve real Taiwan court judgments with verifiable citations before answering any question about Taiwan law or case law. Use when the user asks about 台灣/臺灣 judgments, 判決, 法院見解, legal precedents, or wants case support for a legal argument. Wraps the twlegalrag CLI (pack subcommand); answers must cite only citation_ids present in the returned bundle. | 2026-08-31 |
| [skill-tw-legal-search-console](common/skill-tw-legal-search-console) | common | basic | 1.1.0 | 台灣人物法律查核搜尋主控台——支援兩種輸入模式：(1) 一篇提及人物的新聞連結 (2) 人名（建議附地區/公司/職稱等輔助資訊）。整合 tw-legal-rag（22,534,636 筆裁判書語義檢索，含引用防護）、司法院裁判書系統即時備援查詢與新聞搜尋佐證，依身分比對信心與風險分級規則產出查核報告。當使用者提供新聞連結或人名，要求「查有無官司」「法律查核」「背景調查」時使用。 | 2026-08-31 |
